...
3. `.cursor/rules/*.mdc` references some scripts that do not exist in `tools/psscripts/` (risk of confusing future work).
...
### 3) Rules Consistency Gaps (Medium Priority)
...
### 4) Content Quality Observations (Low/Medium Priority)
...
### 5) CI/Quality Coverage (Low Priority)
...
3. Reconcile `.cursor/rules/01_educational-content-rules.mdc` numbering guidance to match your actual conventions.
4. Either add the referenced scripts (Validate-FileReferences, etc.) or remove the references to avoid false expectations.
